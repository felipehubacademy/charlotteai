...
function buildSystemPrompt(
  ...
  const nextPending = (level === 'Novice' && nextObjectiveId !== undefined)
    ? gc.objectives.find(o => o.id === nextObjectiveId)
    : undefined;
  const nextPendingBlock = nextPending
    ? `\n\nNEXT PENDING OBJECTIVE (HIGHEST PRIORITY — OVERRIDES robotic max-5-words rule when transition is needed): id=${nextPending.id}, label="${nextPending.label_pt || nextPending.label_en || ''}", hint="${nextPending.hint_en || nextPending.hint_pt || ''}", hidden_prompt="${nextPending.hidden_prompt}".\nYour next utterance MUST steer student toward THIS obj:\n- If the obj asks the STUDENT to PRODUCE a statement: ask the question that elicits the hint's structure. Ex: hint="I wasn't happy" → ask "Were you happy?". Keep it short.\n- If the obj is user-asks (student must ask YOU something specific), your reply MUST contain a SHORT TRANSITION CUE telling them WHAT to ask, WITHOUT asking it yourself. Format: "<brief ack>. Now ask about <topic>." Ex: prev obj just met, next hidden_prompt="user asks 'Can you speak + LANGUAGE'" → "Yes I can! Now ask about a language I might speak." Ex: next hidden_prompt="user uses 'By the way' + question" → "Cool. Now try changing the topic with 'By the way...'". This DIRECTS student without stealing the move.\nDO NOT skip ahead. DO NOT ask the student's question yourself. DO NOT just say "Sure!" or "Nice." — that leaves student stuck.`
    : '';
