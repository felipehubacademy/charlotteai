...
function buildSystemPrompt(
  ...
  const nextPendingBlock = nextPending
    ? `\n\nNEXT PENDING OBJECTIVE: id=${nextPending.id}, label="${nextPending.label_pt || nextPending.label_en || ''}", hint="${nextPending.hint_en || nextPending.hint_pt || ''}", hidden_prompt="${nextPending.hidden_prompt}".\nYour next utterance MUST steer student toward THIS obj:\n- If the obj asks the STUDENT to PRODUCE a statement: ask the question that elicits the hint's structure. Ex: hint="I wasn't happy" → ask "Were you happy?".\n- If the obj is user-asks (student must ask YOU something specific), give a SHORT TRANSITION CUE telling them WHAT to ask, WITHOUT asking it yourself. Ex: hidden_prompt="user asks 'Can you speak + LANGUAGE'" → cue: "Now, ask about a language I might speak." NOT "Can you speak Spanish?". Ex: hidden_prompt="user uses 'By the way' + question" → cue: "Try changing the topic with 'By the way...'". This DIRECTS student without stealing the move.\nDO NOT skip ahead. DO NOT ask the student's question yourself.`
    : '';
